...
export default function DealerManagement({ 
  ...
  const handleAddressSearch = (index) => {
    openPostcodeSearch((data) => {
      ...
      // 값이 변경이 안됨 문제: 최신 dealerList를 반영해서 강제로 상태를 업데이트하도록 수정
      onDealerListChange(dealerList.map((dealer, i) => {
        if (i === index) {
          return {
            ...dealer,
            ZIP: data.zonecode,
              ...
              <tr key={index}>
                ...
                <div className="address-input-group">
                  ...
                  <div className="input-with-search">
                    ...
                    <button className="btn btn--dark" type="button" onClick={() => handleAddressSearch(index)} style={{ width: "50px" }} disabled={!isRowEditing(index)}>검색</button>
                    <div className="input w400">
                      <input 
                        type="text" 
                        className={`input__field ${getFieldClassName(index)}`} 
                        placeholder="상세주소"
                        value={dealer.ADDR2 || ''}
                        onChange={(e) => handleFieldChange(index, 'ADDR2', e.target.value)}
                        readOnly={!isRowEditing(index)}
                      />
                      <div className="input__utils">
                        <button
                          type="button"
                          className="jsInputClear input__clear ico ico--input-delete"
                          onClick={() => handleFieldChange(index, 'ADDR2', '')}
                        >
                          상세주소
                        </button>
                      </div>
