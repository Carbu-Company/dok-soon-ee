...
import ListPage from "@/app/(main)/car-goods/cost/ListPage2";
...
async function searchGoodsFeeList(searchParamsWithPage) {
  ...
  try {
    ...
    console.log('서버 액션 결과*******************:', { 
      goodsFeeListLength: result?.data?.goodsFeeList?.length, 
...
// Server Action 정의
async function searchGoodsFeeSummary(searchParamsWithPage) {
  "use server";
...
export default async function CarSelList() {
  ...
  */
  ...
  const goodsFeeSummary = await searchGoodsFeeSummary({ ...defaultParams, ...searchParams });
