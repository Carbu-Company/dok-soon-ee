...
import SellRemoveModal from "@/components/modal/SellRemoveModal";
...
export default function SalesVehicleList(props) {
  ...
  // 매입취소/삭제 모달 관련 state
  const [isSellRemoveModalOpen, setIsSellRemoveModalOpen] = useState(false);
  ...
  // 판매차량 취소/삭제 모달 관련 핸들러
  const handleSellRemoveModalOpen = (car, type) => {
    setSelectedCarForRemove(car);
    setSelectedCarTypeForRemove(type);
    setIsSellRemoveModalOpen(true);
  };

  const handleSellRemoveModalClose = () => {
    setIsSellRemoveModalOpen(false);
    setSelectedCarForRemove(null);
  };

  const handleSellRemoveConfirm = async () => {
    // TODO: 실제 판매차량 취소/삭제 API 호출 구현
    console.log("판매차량 취소/삭제 확인:", selectedCarForRemove);
    // API 호출 후 성공하면 모달 닫기 및 목록 새로고침
    handleSellRemoveModalClose();
  ...
  return (
    <main className="container container--page">
        ...
        <table className="table">
          ...
          <tbody>
            ...
            <tr key={index}>
              ...
              <td>{car.SALE_TP_NM}</td>
              <td>{car.SEL_DLR_NM}</td>
              <td>{car.BUYER_NM}</td>
              <td>{car.SALE_AMT.toLocaleString()}</td>
              <td>{car.PERF_INFE_AMT.toLocaleString()}</td>
              <td>{car.AGENT_SEL_COST.toLocaleString()}</td>
              <td>{car.CAR_SALE_DT}</td> 
                    ...
                    <ul className="select__menu">
                      ...
                      <button
                          ...
                          onClick={e => {
                            e.preventDefault();
                            e.stopPropagation();
                            handleSellRemoveModalOpen(car, "cancel");
                          }}
                          style={{
                            border: "none",
                            background: "none",
      ...
      <SellRemoveModal
        ...
        open={isSellRemoveModalOpen}
        onClose={handleSellRemoveModalClose}
        onConfirm={handleSellRemoveConfirm}
